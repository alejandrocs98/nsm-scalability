...
class CR:
    ...
    def init_params(self, sample=False):
        ...
        # death rate of species
        d_s = g_inv(0.5 * np.ones(self.n_s))
        d_s_std = np.ones_like(d_s)

        # list of parameters
        params = [C, P, d_m, d_s]
        params_std = [C_std, P_std, d_m_std, d_s_std]
    ...
    def update_hypers(self, n_sample=100):
        ...
        for j in range(self.n_obs):
            ...
            try:
                ...
                print(f"Singular matrix for output {j}")
